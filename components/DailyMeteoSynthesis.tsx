  ...
  return (
    <div className="p-4 md:p-6 space-y-6 animate-fadeIn max-w-[1600px] mx-auto">
      <FilterContainer>
        ...
        {/* Ngày so sánh (New) */}
        <div className="flex flex-col gap-1.5">
           <label className="text-xs font-bold text-blue-500 uppercase flex items-center gap-1 ml-1"><ArrowLeftRight size={12} /> Ngày tham chiếu</label>
           <div className="flex items-center gap-1 bg-blue-50/50 border border-blue-100 rounded-lg p-1">
      ...
      <MainCard>
        <PageHeader title="Yếu tố Khí tượng & Xu thế nhiệt độ" subtitle={`${selectedGroup || 'Tất cả các đài'} • ${date.split('-').reverse().join('/')} vs ${compareDate.split('-').reverse().join('/')}`} icon={Clock} iconColorClass="bg-emerald-600" />
           ...
           <table className="w-full text-left border-collapse border-slate-300">
              <thead className="sticky top-0 z-30 bg-white">
                ...
                <tr className="bg-slate-50 sticky top-[30px] z-30 shadow-sm">
                   ...
                   <th className="p-1.5 text-[11px] font-black text-red-400 border border-slate-300 text-center bg-red-50/5 w-[35px]">ΔTmax</th>
                   <th className="p-1.5 text-[11px] font-bold text-red-700 border border-slate-300 text-center bg-red-50/10 w-[45px]">T.Min</th>
                   <th className="p-1.5 text-[11px] font-black text-red-400 border border-slate-300 text-center bg-red-50/5 w-[35px]">ΔTmin</th>
                   
                   {/* Ẩm độ headers */}
                   {['U.Tb', 'U.Min'].map(h => <th key={h} className="p-1.5 text-[11px] font-bold text-blue-700 border border-slate-300 text-center bg-blue-50/10 w-[55px]">{h}</th>)}
                   
                   {/* Mưa headers (Thu hẹp) */}
                   {['Đêm', 'Ngày', '24h'].map(h => <th key={h} className="p-1.5 text-[11px] font-bold text-emerald-700 border border-slate-300 text-center bg-emerald-50/10 w-[60px]">{h}</th>)}
                   
                   {/* Gió headers */}
                   {['Hướng', 'Tốc độ'].map(h => <th key={h} className="p-1.5 text-[11px] font-bold text-sky-700 border border-slate-300 text-center bg-sky-50/10 w-[45px]">{h}</th>)}
                </tr>
              </thead>
