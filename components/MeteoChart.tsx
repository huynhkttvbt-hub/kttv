...
const MeteoChart: React.FC<MeteoChartProps> = ({ data, factor }) => {
  const chartData = React.useMemo(() => {
    return data.map((d) => ({
      ...d,
      displayDate: d.Ngay
        ? d.Ngay.split("-").reverse().slice(0, 2).join("/")
        : "",
    }));
  }, [data]);
  ...
  const renderChartContent = () => {
    switch (factor) {
        ...
        return (
          <div className="flex flex-row h-full gap-4">
            ...
            <div className="flex-1 min-w-0 h-full bg-white rounded-xl border border-slate-200 p-2 relative">
              <span className="absolute top-2 left-2 text-[9px] font-black text-red-500 uppercase z-10 bg-white/90 px-2 py-0.5 rounded shadow-sm border border-red-100">
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis
                    fontSize={9}
                    tick={{ fill: "#ef4444", fontWeight: 700 }}
                    axisLine={false}
                    tickLine={false}
                    domain={["auto", "auto"]}
                  />
            ...
            {/* Biểu đồ Ẩm độ */}
            <div className="flex-1 min-w-0 h-full bg-white rounded-xl border border-slate-200 p-2 relative">
              <span className="absolute top-2 left-2 text-[9px] font-black text-blue-500 uppercase z-10 bg-white/90 px-2 py-0.5 rounded shadow-sm border border-blue-100">
                Diễn biến Ẩm độ (%)
              </span>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                ...
                >
                  ...
                  <YAxis
                    fontSize={9}
                    tick={{ fill: "#3b82f6", fontWeight: 700 }}
                    axisLine={false}
                    tickLine={false}
                    domain={[0, 100]}
                  />
      ...
      case MeteoFactor.KHI_AP:
        return (
          <div className="w-full h-full bg-white rounded-xl border border-slate-200 p-2 relative">
            <span className="absolute top-2 left-2 text-[9px] font-black text-violet-500 uppercase z-10 bg-white/90 px-2 py-0.5 rounded shadow-sm border border-violet-100">
              Khí áp mực biển (mb)
            </span>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
                data={chartData}
                margin={{ top: 35, right: 20, bottom: 0, left: 0 }}
              >
                <CartesianGrid stroke="#f1f5f9" vertical={false} />
                <XAxis
                  dataKey="displayDate"
      ...
      case MeteoFactor.MUA:
        return (
          <div className="w-full h-full bg-white rounded-xl border border-slate-200 p-2 relative">
            <span className="absolute top-2 left-2 text-[9px] font-black text-emerald-500 uppercase z-10 bg-white/90 px-2 py-0.5 rounded shadow-sm border border-emerald-100">
              Lượng mưa ngày (mm)
            </span>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
      ...
      case MeteoFactor.GIO:
        return (
          <div className="w-full h-full bg-white rounded-xl border border-slate-200 p-2 relative">
            <span className="absolute top-2 left-2 text-[9px] font-black text-sky-500 uppercase z-10 bg-white/90 px-2 py-0.5 rounded shadow-sm border border-sky-100">
              Diễn biến Tốc độ gió (m/s)
            </span>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
                data={windData}
              ...
              >
                ...
                <YAxis
                  fontSize={9}
                  tick={{ fill: "#0ea5e9", fontWeight: 700 }}
                  axisLine={false}
                  tickLine={false}
                />
                <Tooltip
  ...
  return <div className="w-full h-[250px] mt-4">{renderChartContent()}</div>;
};
