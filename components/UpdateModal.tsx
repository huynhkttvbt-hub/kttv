...
const UpdateModal: React.FC<UpdateModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        alert(
          "Cập nhật thành công! Các giá trị đặc trưng (Hmax, Hmin, Htb) đã được tính toán lại."
        );
        onSave();
        onClose();
      }
    } catch (err: any) {
      // Hiển thị lỗi chi tiết thay vì [object Object]
      alert(
        `LỖI LƯU DỮ LIỆU: ${
          err.message || "Hãy kiểm tra cấu hình Database hoặc quyền truy cập."
        }`
      );
    } finally {
      setLoading(false);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fadeIn">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-slideUp border border-slate-200">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, TenDai: e.target.value })
                  }
