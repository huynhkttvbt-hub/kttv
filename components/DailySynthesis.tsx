...
const DailySynthesis: React.FC = () => {
  ...
  const handleExportExcel = () => {
     ...
     groupedData.forEach(group => {
       group.stations.forEach(d => {
         flatData.push({
           'Đài': d.TenDai, 'Trạm': d.TenTram, 'Ngày': d.Ngay,
           '01h': d['01h'] ?? '', '07h': d['07h'] ?? '', '13h': d['13h'] ?? '', '19h': d['19h'] ?? '',
           'Hmax': d.Hmax ?? '', 'Hmin': d.Hmin ?? '', 'Htb': d.Htb ?? '',
           'R1': d.R1 ?? '', 'R7': d.R7 ?? '', 'R13': d.R13 ?? '', 'R19': d.R19 ?? '', 'R24': d.R24 ?? ''
         });
       });
     });
  ...
  const renderCell = (val: any, isRain: boolean = false) => {
    ...
    return <span className={isRain ? "text-emerald-700 font-black" : "text-blue-700 font-black"}>{val}</span>;
  };
...
>>>>>>> 120d3ae5a591056c914629f173d33a88a361258e
        ...
        <div className="flex flex-col gap-1.5">
           ...
           <div className="flex items-center gap-1 bg-blue-50/50 border border-blue-100 rounded-lg p-1">
              <button onClick={() => adjustDate(-1)} className="p-1.5 hover:bg-blue-100 rounded-md text-blue-600 transition-colors"><ChevronLeft size={14} /></button>
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="bg-transparent border-none text-[11px] font-black text-blue-900 focus:ring-0 w-[110px] text-center outline-none cursor-pointer" />
              <button onClick={() => adjustDate(1)} className="p-1.5 hover:bg-blue-100 rounded-md text-blue-600 transition-colors"><ChevronRight size={14} /></button>
           </div>
        </div>
           ...
           <table className="w-full text-left border-collapse border-slate-300">
              <thead className="sticky top-0 z-30 bg-white">
                <tr className="bg-slate-50">
                   <th rowSpan={2} className="p-3 text-[10px] font-black text-slate-700 uppercase border border-slate-300 min-w-[150px] sticky left-0 bg-slate-100 z-30 shadow-[1px_0_0_0_#cbd5e1]">ĐƠN VỊ</th>
                   <th colSpan={7} className="p-2 text-[10px] font-black text-blue-600 uppercase border border-slate-300 text-center bg-blue-50/30">Mực nước (cm)</th>
                   <th colSpan={5} className="p-2 text-[10px] font-black text-emerald-600 uppercase border border-slate-300 text-center bg-emerald-50/30">Mưa (mm)</th>
                </tr>
                <tr className="bg-slate-50 sticky top-[30px] z-30 shadow-sm">
                   {['01h', '07h', '13h', '19h', 'Hmax', 'Hmin', 'Htb'].map(h => (
                     <th key={h} className={`p-2 text-[9px] font-black border border-slate-300 text-center min-w-[50px] ${h.startsWith('H') ? 'text-blue-700 bg-blue-50/20' : 'text-slate-500 bg-slate-50'}`}>{h}</th>
                   ))}
                   {['R1', 'R7', 'R13', 'R19', 'R24'].map(h => (
                     <th key={h} className="p-2 text-[9px] font-black text-emerald-700 border border-slate-300 text-center min-w-[50px] bg-emerald-50/20">{h}</th>
                   ))}
                </tr>
              </thead>
                   ...
                   groupedData.map((group) => (
                     <React.Fragment key={group.groupName}>
                       <tr className="bg-slate-100">
                         <td colSpan={13} className="p-2.5 font-black text-slate-800 uppercase text-[10px] border border-slate-300 sticky left-0 z-20 bg-slate-200 pl-3 shadow-[1px_0_0_0_#cbd5e1]">
                           {group.groupName}
                         </td>
                       </tr>
                       {group.stations.map((row, idx) => (
                         <tr key={`${group.groupName}-${idx}`} className="hover:bg-blue-50/30 transition-colors group">
                            ...
                            <td className="p-2 text-center border border-slate-300">{renderCell(row['01h'])}</td>
                            <td className="p-2 text-center border border-slate-300">{renderCell(row['07h'])}</td>
                            <td className="p-2 text-center border border-slate-300">{renderCell(row['13h'])}</td>
                            <td className="p-2 text-center border border-slate-300">{renderCell(row['19h'])}</td>
                            <td className="p-2 text-center border border-slate-300 bg-blue-50/5">{renderCell(row.Hmax)}</td>
                            <td className="p-2 text-center border border-slate-300 bg-blue-50/5">{renderCell(row.Hmin)}</td>
                            <td className="p-2 text-center border border-slate-300 bg-blue-50/5">{renderCell(row.Htb)}</td>
                            <td className="p-2 text-center border border-slate-300">{renderCell(row.R1, true)}</td>
                            <td className="p-2 text-center border border-slate-300">{renderCell(row.R7, true)}</td>
                            <td className="p-2 text-center border border-slate-300">{renderCell(row.R13, true)}</td>
                            <td className="p-2 text-center border border-slate-300">{renderCell(row.R19, true)}</td>
                            <td className="p-2 text-center border border-slate-300 bg-emerald-50/5">{renderCell(row.R24, true)}</td>
                         </tr>
                       ))}
                     </React.Fragment>
                   ))
