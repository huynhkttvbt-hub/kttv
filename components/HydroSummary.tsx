...
const HydroSummary: React.FC = () => {
    ...
    const loadMetadata = async () => {
      ...
      if (data.length > 0) {
        const uniqueGroups = Array.from(
          new Set(data.map((m) => m.TenDai).filter(Boolean))
        ) as string[];
  ...
  const fetchSummaryData = async () => {
    ...
    try {
      ...
      if (period === "DAY") {
        ...
        const endDay =
          period === "T1"
            ...
            ? "20"
            : period === "T3"
            ? lastDayDate.getDate().toString().padStart(2, "0")
            : lastDayDate.getDate().toString().padStart(2, "0");

        query = query
          .gte("Ngay", `${selectedYear}-${mStr}-${startDay}`)
          .lte("Ngay", `${selectedYear}-${mStr}-${endDay}`);
      }

      const { data, error: sbError } = await query.order("Ngay", {
        ascending: true,
      });
      ...
      const monthForTbnn =
        period === "DAY"
          ? new Date(specificDate).getMonth() + 1
          : selectedMonth;
  ...
  const processedStats = useMemo(() => {
    ...
    const hMaxArr = rawStats
      .map((d) => parseFloat(d.Hmax))
      .filter((v) => !isNaN(v));
    const hMinArr = rawStats
      .map((d) => parseFloat(d.Hmin))
      .filter((v) => !isNaN(v));
    const hTbArr = rawStats
      .map((d) => parseFloat(d.Htb))
      .filter((v) => !isNaN(v));
    const rainArr = rawStats
      .map((d) => parseFloat(d.R24))
      .filter((v) => !isNaN(v));

    const hMax = hMaxArr.length ? Math.max(...hMaxArr) : 0;
    const hMin = hMinArr.length ? Math.min(...hMinArr) : 0;
    const hTb = hTbArr.length
      ? hTbArr.reduce((a, b) => a + b, 0) / hTbArr.length
      : 0;
    const rainSum = rainArr.reduce((a, b) => a + b, 0).toFixed(1);
    const rainMax = rainArr.length ? Math.max(...rainArr) : 0;

    const maxDay =
      rawStats.find((d) => parseFloat(d.Hmax) === hMax)?.Ngay || "-";
    const minDay =
      rawStats.find((d) => parseFloat(d.Hmin) === hMin)?.Ngay || "-";
    const ngayRainMax =
      rawStats.find((d) => parseFloat(d.R24) === rainMax)?.Ngay || "-";
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `DacTrung_${selectedStation}_${selectedMonth}_${selectedYear}.xlsx`
    );
  };
  ...
  const ComparisonBadge = ({
    ...
    if (target === null || target === undefined)
      return <span className="text-[9px] text-slate-300 italic">--</span>;
    ...
    return (
      <div
        className={`flex items-center gap-1 text-[10px] font-black px-1.5 py-0.5 rounded-full ${
          isHigher ? "bg-red-50 text-red-600" : "bg-blue-50 text-blue-600"
        }`}
      >
          ...
          <div className="p-4 overflow-x-auto">
            <table className="w-full text-left border-separate border-spacing-0 border border-slate-300 rounded-lg overflow-hidden">
              <thead className="bg-slate-100">
                <tr>
                  <th className="p-3 text-[10px] font-black text-slate-500 uppercase border-r border-b border-slate-300 text-center">
                    Yếu tố
                  </th>
                  <th className="p-3 text-[10px] font-black text-slate-500 uppercase border-r border-b border-slate-300 text-center">
                    Thực đo
                  ...
                  <th className="p-3 text-[10px] font-black text-slate-500 uppercase border-b border-slate-300 text-center">
                    Thời gian xuất hiện
                  </th>
                </tr>
              </thead>
                ...
                {processedStats ? (
                  <>
                    <tr className="hover:bg-slate-50 transition-colors">
                      <td className="p-4 text-xs font-bold text-slate-600 border-r border-b border-slate-300 uppercase">
                        Hmax (cm)
                      </td>
                      ...
                      <td className="p-4 text-center border-r border-b border-slate-300 text-xs font-bold text-slate-500">
                        {tbnnData?.Hmax ?? "-"}
                      </td>
                      <td className="p-4 border-r border-b border-slate-300">
                        <div className="flex items-center justify-center w-full h-full">
                          <ComparisonBadge
                            current={processedStats.hMax}
                            target={tbnnData?.Hmax ?? null}
                          />
                        </div>
                      </td>
                      <td className="p-4 text-center border-b border-slate-300 text-[11px] font-bold text-slate-500">
                        {processedStats.maxDay.split("-").reverse().join("/")}
                      </td>
                    </tr>
                    <tr className="hover:bg-slate-50 transition-colors">
                      <td className="p-4 text-xs font-bold text-slate-600 border-r border-b border-slate-300 uppercase">
                      ...
                      <td className="p-4 border-r border-b border-slate-300">
                        <div className="flex items-center justify-center w-full h-full">
                          <ComparisonBadge
                            current={processedStats.hMin}
                            target={tbnnData?.Hmin ?? null}
                          />
                        </div>
                      </td>
                      <td className="p-4 text-center border-b border-slate-300 text-[11px] font-bold text-slate-500">
                        {processedStats.minDay.split("-").reverse().join("/")}
                      </td>
                    </tr>
                    <tr className="hover:bg-slate-50 transition-colors">
                      <td className="p-4 text-xs font-bold text-slate-600 border-r border-b border-slate-300 uppercase">
                      ...
                      <td className="p-4 border-r border-b border-slate-300">
                        <div className="flex items-center justify-center w-full h-full">
                          <ComparisonBadge
                            current={processedStats.hTb}
                            target={tbnnData?.Htb ?? null}
                          />
                        </div>
                      </td>
                      <td className="p-4 text-center border-b border-slate-300 text-[11px] font-bold text-slate-400">
                        ({processedStats.count} ngày)
                      </td>
                    </tr>
                    <tr className="bg-emerald-50/30 hover:bg-emerald-50 transition-colors">
                      <td className="p-4 text-xs font-bold text-emerald-800 border-r border-slate-300 uppercase">
                        Tổng mưa (mm)
                      </td>
                      <td className="p-4 text-center border-r border-slate-300">
                        <span className="text-sm font-black text-emerald-600">
                          {processedStats.rainSum}
                        </span>
                      </td>
                      <td className="p-4 text-center border-r border-slate-300 text-xs font-bold text-slate-500">
                        {tbnnData?.Rtb ?? "-"}
                      </td>
                      <td className="p-4 border-r border-slate-300">
                        <div className="flex items-center justify-center w-full h-full">
                          <ComparisonBadge
                            current={parseFloat(processedStats.rainSum)}
                            target={tbnnData?.Rtb ?? null}
                            unit="mm"
                          />
                        </div>
                      </td>
                      <td className="p-4 text-center text-[10px] font-black text-emerald-400 uppercase tracking-tighter">
                        TOÀN THỜI KỲ
                      </td>
                    </tr>
                  </>
                  ...
                  <tr>
                    <td colSpan={5} className="p-20 text-center">
                      <LoadingState message="Chưa có dữ liệu" />
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <MainCard className="p-6">
          ...
          <div className="mt-4 p-3 bg-slate-50 rounded-xl border border-slate-100">
            <div className="flex justify-between items-center mb-1">
              <span className="text-[9px] font-black text-slate-400 uppercase text-xs">
                Lượng mưa lớn nhất
              </span>
              <span className="text-[10px] font-bold text-slate-700 text-sm">
                {processedStats?.rainMax !== undefined
                  ? `${processedStats.rainMax} mm`
                  : "N/A"}
              </span>
            </div>
