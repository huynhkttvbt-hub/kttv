...
import { MeteoData, StationMetadata, TBNNData } from '../types';
...
const MeteoSummary: React.FC = () => {
  ...
  useEffect(() => {
    const loadMeta = async () => {
      ...
      setMetadata(meta);
      const groups = Array.from(new Set(meta.map(m => m.TenDai).filter(Boolean))) as string[];
      if (groups.length > 0) setSelectedGroup(''); 
    };
    loadMeta();
  }, []);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const availableGroups: string[] = selectedGroup 
        ? [selectedGroup] 
      ...
      availableGroups.forEach(groupName => {
        ...
        const summaries: MeteoStationSummary[] = stationMetas.map(meta => {
          const rows = meteoRows.filter(r => r.Tram === meta.TenTram || r.tram === meta.TenTram);
  ...
  useEffect(() => {
    fetchData();
  }, [selectedGroup, selectedMonth, selectedYear, period, metadata]);
      ...
      <FilterContainer>
        <div className="flex flex-col gap-1.5 w-[160px]">
          ...
          <select value={selectedGroup} onChange={(e) => setSelectedGroup(e.target.value)} className="w-full bg-blue-50/50 border border-blue-100 rounded-lg p-2.5 text-xs font-bold text-blue-800 outline-none cursor-pointer">
            ...
            {Array.from(new Set(metadata.map(m => m.TenDai).filter(Boolean))).sort().map(g => <option key={g} value={g}>{g}</option>)}
          </select>
        </div>
                  ...
                  <React.Fragment key={group}>
                    ...
                    {stations.map((s, idx) => (
                      <tr key={idx} className="hover:bg-blue-50/30 transition-colors border border-slate-300 group">
                        ...
                        <td className="p-2.5 text-center border border-slate-300 font-bold text-slate-900">{s.HasData ? s.Ttb : '-'}</td>
                        <td className="p-2.5 text-center border border-slate-300 font-black text-red-600">{s.HasData ? s.Tx : '-'}</td>
                        <td className="p-2.5 text-center border border-slate-300 text-[10px] text-slate-600 font-bold italic">{formatDate(s.NgayTx)}</td>
                        <td className="p-2.5 text-center border border-slate-300 font-black text-blue-600">{s.HasData ? s.Tn : '-'}</td>
                        ...
                        <td className="p-2.5 text-center border border-slate-300 text-slate-900 font-bold">{s.Utb}</td>
                        <td className="p-2.5 text-center border border-slate-300 font-black text-orange-600">{s.Umin}</td>
                        ...
                        <td className="p-2.5 text-center border border-slate-300 text-slate-700 font-black">{s.TbnnTemp ?? '-'}</td>
                        ...
                        <td className={`p-2.5 text-center border border-slate-300 font-black ${s.CompareTempDiff !== null && s.CompareTempDiff > 0 ? 'text-red-700' : s.CompareTempDiff !== null && s.CompareTempDiff < 0 ? 'text-blue-700' : 'text-slate-400'}`}>
                          {s.CompareTempDiff !== null ? (s.CompareTempDiff > 0 ? `+${s.CompareTempDiff}` : s.CompareTempDiff) : '-'}
                        </td>
