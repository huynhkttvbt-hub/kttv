...
const MeteoTable: React.FC<MeteoTableProps> = ({ data, loading, factor }) => {
  ...
  const cell = (val: any, bold = false, colorClass = "text-slate-800") => (
    <td
      ...
      } ${colorClass} text-xs md:text-[12px]`}
    >
  ...
  const weatherCell = (val: any, isPast: boolean = false) => {
    ...
    return (
      <td
        className={`p-2 border border-slate-300 text-center text-[11px] md:text-xs leading-tight ${
          isSpecial
            ? `${textColorClass} font-bold ${bgColorClass}`
  ...
  const renderTableHeader = () => {
    const headerClass =
      "py-1 px-1 font-bold text-slate-700 border border-slate-300 text-center text-[10px] md:text-xs uppercase";
    const subHeaderClass =
      "py-1 px-1 text-[8px] md:text-[9px] font-bold border border-slate-300 text-center";
        ...
        return (
          <>
            <tr className="bg-slate-50">
              <th
                rowSpan={2}
                className={`${headerClass} w-[100px] sticky left-0 z-40 bg-slate-100 shadow-[1px_0_0_0_#cbd5e1] text-slate-900`}
              >
                Ngày
              </th>
              <th
                colSpan={11}
                className={`${headerClass} text-red-600 bg-red-50/30`}
              >
                Nhiệt độ (°C)
              </th>
              <th
                colSpan={10}
                className={`${headerClass} text-blue-600 bg-blue-50/30`}
              >
                Ẩm độ (%)
              </th>
            </tr>
            <tr className="bg-slate-50 sticky top-[20px] z-30 shadow-sm">
              {[
                ...
                <th
                  key={`t-${h}`}
                  className={`${subHeaderClass} text-red-700 bg-red-50/10 w-[45px]`}
                >
                  {h}
                </th>
              ))}
              {[
                ...
                <th
                  key={`u-${h}`}
                  className={`${subHeaderClass} text-blue-700 bg-blue-50/10 w-[45px]`}
                >
                  {h}
                </th>
              ))}
        ...
        return (
          <>
            <tr className="bg-slate-50">
              ...
              <th
                colSpan={9}
                className={`${headerClass} text-indigo-600 bg-indigo-50/30`}
              >
                Khí áp trạm (mb)
              </th>
              <th
                colSpan={9}
                className={`${headerClass} text-violet-600 bg-violet-50/30`}
              >
                Khí áp biển (mb)
              </th>
            </tr>
            <tr className="bg-slate-50 sticky top-[20px] z-30 shadow-sm">
              {["1h", "4h", "7h", "10h", "13h", "16h", "19h", "22h", "TB"].map(
                (h) => (
                  <th
                    key={`pt-${h}`}
                    className={`${subHeaderClass} text-indigo-700 bg-indigo-50/10 w-[55px]`}
                  >
                    Pt{h}
                  </th>
                )
              ...
              {["1h", "4h", "7h", "10h", "13h", "16h", "19h", "22h", "TB"].map(
                (h) => (
                  <th
                    key={`pb-${h}`}
                    className={`${subHeaderClass} text-violet-700 bg-violet-50/10 w-[55px]`}
                  >
                    Pb{h}
                  </th>
                )
        ...
        return (
          <>
            <tr className="bg-slate-50">
              <th
                rowSpan={3}
                className={`${headerClass} w-[100px] sticky left-0 z-50 bg-slate-100 shadow-[1px_0_0_0_#cbd5e1] text-slate-900`}
              >
                Ngày
              </th>
              <th
                colSpan={16}
                className={`${headerClass} text-sky-600 bg-sky-50/40 border-b-sky-200`}
              >
                Gió Các Obs
              </th>
              <th
                colSpan={8}
                className={`${headerClass} text-orange-600 bg-orange-50/40 border-b-orange-200`}
              >
                Gió giật
              </th>
            </tr>
            <tr className="bg-slate-50 sticky top-[20px] z-40">
              {obsHours.map((h) => (
                <th
                  key={`obs-${h}`}
                  colSpan={2}
                  className={`${subHeaderClass} text-sky-700 bg-sky-50/20 w-[90px]`}
                >
                  {h}
                </th>
              ))}
              {maxHours.map((h) => (
                <th
                  key={`max-${h}`}
                  colSpan={2}
                  className={`${subHeaderClass} text-orange-700 bg-orange-50/20 w-[90px]`}
                >
                  {h}
                </th>
              ))}
            ...
            <tr className="bg-slate-50 sticky top-[30px] z-40 shadow-sm">
              {Array.from({ length: 12 }).map((_, i) => (
                <React.Fragment key={i}>
                  <th
                    className={`${subHeaderClass} text-slate-500 bg-slate-50 w-[45px] font-medium border-t-0`}
                  >
                    H
                  </th>
                  <th
                    className={`${subHeaderClass} text-slate-900 bg-slate-50 w-[45px] font-black border-t-0`}
                  >
                    T
                  </th>
                </React.Fragment>
      ...
      case MeteoFactor.MUA:
        return (
          <tr className="bg-slate-50">
            ...
            <th
              className={`${headerClass} text-emerald-600 bg-emerald-50/10 w-[70px]`}
            >
              R1h
            </th>
            <th
            ...
            <tr className="bg-slate-50">
              ...
              {wHours.map((h) => (
                <th
                  key={h}
                  colSpan={3}
                  className={`${headerClass} text-amber-600 bg-amber-50/30`}
                >
                  {h}
                </th>
              ))}
            ...
            <tr className="bg-slate-50 sticky top-[20px] z-30 shadow-sm">
              {wHours.map((h) => (
                <React.Fragment key={h}>
                  <th
                    className={`${subHeaderClass} text-slate-500 bg-slate-50 w-[80px]`}
                  >
                    W
                  </th>
                  <th
  ...
  const renderTableBody = () => {
    return data.map((row, idx) => {
      const dateDisplay = row.Ngay
        ? row.Ngay.split("-").reverse().join("/")
        : "-";
      ...
      return (
        <tr key={idx} className="hover:bg-blue-50/40 transition-colors group">
          ...
          {factor === MeteoFactor.HIEN_TUONG && (
            <>
              {weatherCell(row.W1h, false)}
              {weatherCell(row.W11h, true)}
              {weatherCell(row.W21h, true)}
              {weatherCell(row.W4h, false)}
              {weatherCell(row.W14h, true)}
              {weatherCell(row.W24h, true)}
              {weatherCell(row.W7h, false)}
              {weatherCell(row.W17h, true)}
              {weatherCell(row.W27h, true)}
              {weatherCell(row.W10h, false)}
              {weatherCell(row.W110h, true)}
              {weatherCell(row.W210h, true)}
              {weatherCell(row.W13h, false)}
              {weatherCell(row.W113h, true)}
              {weatherCell(row.W213h, true)}
              {weatherCell(row.W16h, false)}
              {weatherCell(row.W116h, true)}
              {weatherCell(row.W216h, true)}
              {weatherCell(row.W19h, false)}
              {weatherCell(row.W119h, true)}
              {weatherCell(row.W219h, true)}
              {weatherCell(row.W22h, false)}
              {weatherCell(row.W122h, true)}
              {weatherCell(row.W222h, true)}
            </>
          )}
        </tr>
      );
  ...
  return (
    <div className="relative overflow-auto max-h-[650px] w-full border border-slate-300 rounded-xl bg-white shadow-sm">
      <table className="w-full text-left border-collapse">
        <thead className="sticky top-0 z-30">{renderTableHeader()}</thead>
        <tbody>
          {renderTableBody()}
          {data.length === 0 && !loading && (
            <tr>
              <td
                colSpan={100}
                className="p-24 text-center bg-white font-bold text-slate-400"
              >
